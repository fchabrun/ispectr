...
if MODE == "XAVIER":
    ...
    MODEL_PATH = r"D:/Anaconda datasets/Capillarys/IF_transformer"
elif MODE == "FLORIS":
    # Floris
    Deep_sup = False
    MODEL_PATH = "C:/Users/admin/Documents/Capillarys/temp2021"
    MODEL_NAME = "seg3d-3D-loss-binary_crossentropy.h5"
    POST_PROCESS_METHOD = ('none','invert_predictions')[0]
    POST_PROCESS_SQUARE = False
    POST_PROCESS_SMOOTH = False
    INPUT_DIMENSIONS = 4

# ces infos sont à prendre en compte uniquement si on change l'architecture du modèle pour les modèles initialement présentés
...
print("Time per sample: "+format(round(1000*(end-start)/size, 3))+'ms') # 3.049 ms per sample
print("Time for 100 samples: "+format(round(100*(end-start)/size, 1))+'s') # 0.3s per 100 samples
